...
from wtforms import IntegerField, DecimalField, PasswordField, SelectField, StringField, SubmitField, TextAreaField
from wtforms.validators import InputRequired, Length, NumberRange, Optional


class LoginForm(FlaskForm):
    username = StringField(validators=[InputRequired(), Length(
        min=4, max=32)], render_kw={"placeholder": "Username"})
    password = PasswordField(validators=[InputRequired(), Length(
        min=8, max=32)], render_kw={"placeholder": "Password"})
...
class ProfessionalRegistrationForm(FlaskForm):
    ...
    service_type = SelectField(
        "Service Name", choices=[], validators=[InputRequired()])
...
class RemarksForm(FlaskForm):
    ...
    remarks = TextAreaField("Remarks (if any)", validators=[Optional()])


class ProfileForm(FlaskForm):
    fullname = StringField("Full Name", render_kw={"readonly": True})
    address = StringField("Address", render_kw={"readonly": True})
    pincode = IntegerField("Pincode", render_kw={"readonly": True})
    contact_number = IntegerField("Contact No", render_kw={"readonly": True})
